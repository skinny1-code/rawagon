...
function commit(creds,keyHex){return'0x'+crypto.createHmac('sha256',Buffer.from(keyHex,'hex')).update(JSON.stringify(creds)).digest('hex');}
function prove(creds,keyHex){return{proof:crypto.createHmac('sha256',Buffer.from(keyHex,'hex')).update(JSON.stringify(creds)).digest('hex'),commitment:commit(creds,keyHex),ts:Date.now()};}
function bioDerive(vec,salt){const s=salt||crypto.randomBytes(32).toString('hex');return{masterKey:crypto.createHmac('sha256',Buffer.from(s,'hex')).update(JSON.stringify(vec)).digest('hex'),salt:s};}
